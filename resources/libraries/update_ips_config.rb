module Rbmonitor
  module Helpers
    ...
    def monitors_ips_ids(resource_node,inserted)
     ...
     node.default["redborder"]["monitor"]["config"][:sensors].push("/* IDS/IPS monitoring: */")
       node["redborder"]["snort"]["groups"].each do |id, original_group|
         ...
         node.default["redborder"]["monitor"]["config"][:sensors].push("/* snort group monitoring #{group_name}  (manager group_id: #{group_id}; internal group_id:  #{id} ) Total monitors:  #{original_group["monitors"].size} */")
    ...
    def update_ips_config(resource, inserted)
      ...
      node.default["redborder"]["monitor"]["config"][:sensors].push("/* Sensor monitoring - Total monitors: #{node["redborder"]["monitors"].size} */")
